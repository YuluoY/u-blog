 ...
 * @returns { liked, likeCount }
 */
export async function toggleCommentLike(commentId: number, fingerprint?: string): Promise<{ liked: boolean; likeCount: number }>
{
  const res = await instance.post<BackendResponse<{ liked: boolean; likeCount: number }>>('/comment-like', { commentId, fingerprint })
  const payload = res.data
  if (payload.code !== 0)
 ...
 * @param fingerprint 可选的浏览器指纹
 * @returns Record<commentId, liked>
 */
export async function getCommentLikeStatuses(commentIds: number[], fingerprint?: string): Promise<Record<number, boolean>>
{
  const res = await instance.post<BackendResponse<Record<number, boolean>>>('/comment-like-statuses', { commentIds, fingerprint })
  const payload = res.data
  if (payload.code !== 0)
